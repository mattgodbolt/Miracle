...
// ---------------------------------------------------------------------------
// Macro expansion — converts intermediate jscpp to pure JavaScript
// ---------------------------------------------------------------------------
 ...
 */
function extractMacroArgs(code, pos) {
  let depth = 0;
  let current = "";
  const args = [];
...
/**
 * Replace all calls to `macroName(...)` with `expand(args)`.
 * Handles nested parentheses in arguments correctly.
 */
function expandMacro(code, macroName, expand) {
  ...
  while ((match = pattern.exec(code)) !== null) {
    ...
    const [args, end] = extractMacroArgs(code, parenPos);
...
function substituteOutsideComments(code, subs) {
  ...
  return parts
    .map((part, i) => {
      if (i % 2 !== 0) return part; // inside a comment — leave unchanged
...
const REGISTER_SUBS = [
  // 16-bit pseudo-register read expressions
  [/\bBCR\b/g, "(z80.c | (z80.b << 8))"],
  [/\bDER\b/g, "(z80.e | (z80.d << 8))"],
  [/\bHLR\b/g, "(z80.l | (z80.h << 8))"],
  // Split SP/PC byte accessors (before SP/PC simple forms)
  [/\bSPHR\b/g, "(z80.sp >> 8)"],
  [/\bSPLR\b/g, "(z80.sp & 0xff)"],
  [/\bPCHR\b/g, "(z80.pc >> 8)"],
  [/\bPCLR\b/g, "(z80.pc & 0xff)"],
  // Read-aliases for SP and PC
  [/\bSPR\b/g, "z80.sp"],
  [/\bPCR\b/g, "z80.pc"],
  // Multi-letter named registers (before single-letter I, R)
  [/\bIFF1\b/g, "z80.iff1"],
  [/\bIFF2\b/g, "z80.iff2"],
  [/\bIM\b/g, "z80.im"],
  [/\bR7\b/g, "z80.r7"],
  // Alternate registers (before their unprimed equivalents)
  [/\bA_\b/g, "z80.a_"],
  [/\bF_\b/g, "z80.f_"],
  [/\bB_\b/g, "z80.b_"],
  [/\bC_\b/g, "z80.c_"],
...
/**
 * Expand all C-preprocessor macro calls in intermediate jscpp to pure JS.
 *
 * @param {string} code       - Intermediate string produced by z80.mjs
 * @param {string|null} register - 'ix', 'iy', or null (non-DDFD opcodes)
 * @returns {string}          - Pure JavaScript ready to inject into z80_ops.js
 */
export function macroExpand(code, register = null) {
  // 1. Strip GPL license comment blocks and autogenerated notices.
  code = code.replace(
    /\/\*[\s\S]*?(?:Copyright|autogenerated)[\s\S]*?\*\/\s*/g,
    "",
  );

  // 2. Build register-alias substitution list, prepending REGISTER* entries
  //    for DDFD opcodes (ix/iy context) when needed.
  const regSubs = [...REGISTER_SUBS];
  if (register) {
    const rn = register.toLowerCase(); // 'ix' or 'iy'
    const full = `(z80.${rn}l | (z80.${rn}h << 8))`;
    // Prepend so REGISTERH/L/R are tested before REGISTER (all have word
    // boundaries, so ordering is only for readability here).
    regSubs.unshift(
  ...
  // 3. Apply register-alias substitutions (code only, not inside comments).
  code = substituteOutsideComments(code, regSubs);

  // 4. Simple aliases that the old C macros defined as identity substitutions.
  code = code.replace(/\breadbyte_internal\b/g, "readbyte");
  code = code.replace(/\bwritebyte_internal\b/g, "writebyte");

  // 5. contend_io(port, time) — port is irrelevant (no memory contention).
  code = expandMacro(code, "contend_io", ([, time]) => `tstates += (${time});`);

  // 5. Expand parameterised macro calls.  Longer names (ADC16, ADD16, SBC16,
  //    BIT_I, LD16_*) must be expanded before their shorter prefix-siblings.

  code = expandMacro(
    code,
    "ADC16",
    ([v]) =>
      `{ var add16temp = (z80.l | (z80.h << 8)) + (${v}) + (z80.f & FLAG_C);` +
      ` var lookup = (((z80.l | (z80.h << 8)) & 0x8800) >> 11) | (((${v}) & 0x8800) >> 10) | ((add16temp & 0x8800) >> 9);` +
      ` z80.h = (add16temp >> 8) & 0xff; z80.l = add16temp & 0xff;` +
      ` z80.f = (add16temp & 0x10000 ? FLAG_C : 0) | overflow_add_table[lookup >> 4] |` +
      ` (z80.h & (FLAG_3 | FLAG_5 | FLAG_S)) | halfcarry_add_table[lookup & 0x07] |` +
  ...
  code = expandMacro(
    ...
    ([v]) =>
      ...
      ` z80.a = adctemp & 0xff;` +
      ` z80.f = (adctemp & 0x100 ? FLAG_C : 0) | halfcarry_add_table[lookup & 0x07] |` +
      ` overflow_add_table[lookup >> 4] | sz53_table[z80.a]; }`,
  );
  ...
  code = expandMacro(
    ...
    ([v1, v2, v1h, v1l]) =>
      `{ var add16temp = (${v1}) + (${v2});` +
      ` var lookup = (((${v1}) & 0x0800) >> 11) | (((${v2}) & 0x0800) >> 10) | ((add16temp & 0x0800) >> 9);` +
      ` addTstates(7); (${v1h}) = (add16temp >> 8) & 0xff; (${v1l}) = add16temp & 0xff;` +
      ` z80.f = (z80.f & (FLAG_V | FLAG_Z | FLAG_S)) | (add16temp & 0x10000 ? FLAG_C : 0) |` +
      ` ((add16temp >> 8) & (FLAG_3 | FLAG_5)) | halfcarry_add_table[lookup]; }`,
  );
  ...
  code = expandMacro(
    ...
    ([v]) =>
      ...
      ` z80.a = addtemp & 0xff;` +
      ` z80.f = (addtemp & 0x100 ? FLAG_C : 0) | halfcarry_add_table[lookup & 0x07] |` +
      ` overflow_add_table[lookup >> 4] | sz53_table[z80.a]; }`,
  );
  ...
  code = expandMacro(
    ...
    () =>
      ...
      ` calltempl = readbyte(z80.pc++); z80.pc &= 0xffff; addTstates(1);` +
      ` calltemph = readbyte(z80.pc++); z80.pc &= 0xffff;` +
      ` addTstates(6);` +
      ` z80.sp--; z80.sp &= 0xffff; writebyte(z80.sp, z80.pc >> 8);` +
      ` z80.sp--; z80.sp &= 0xffff; writebyte(z80.sp, z80.pc & 0xff);` +
      ` z80.pc = calltempl | (calltemph << 8); }`,
  );
  ...
  code = expandMacro(
    ...
    ([v]) =>
      ...
      ` z80.f = (cptemp & 0x100 ? FLAG_C : (cptemp ? 0 : FLAG_Z)) | FLAG_N |` +
      ` halfcarry_sub_table[lookup & 0x07] | overflow_sub_table[lookup >> 4] |` +
      ` ((${v}) & (FLAG_3 | FLAG_5)) | (cptemp & FLAG_S); }`,
  );
  ...
  code = expandMacro(
    ...
    ([v]) =>
      `{ (${v}) = ((${v}) + 1) & 0xff;` +
      ` z80.f = (z80.f & FLAG_C) | ((${v}) === 0x80 ? FLAG_V : 0) |` +
      ` ((${v}) & 0x0f ? 0 : FLAG_H) | sz53_table[${v}]; }`,
  );
  ...
  code = expandMacro(
    ...
    "IN",
    ([reg, port]) =>
      `{ tstates += 3; (${reg}) = readport((${port}));` +
      ` z80.f = (z80.f & FLAG_C) | sz53p_table[(${reg})]; }`,
  );
  ...
  code = expandMacro(
    ...
    "JP",
    () =>
      `{ var jptemp = z80.pc; var pcl = readbyte(jptemp++); jptemp &= 0xffff;` +
      ` var pch = readbyte(jptemp); z80.pc = pcl | (pch << 8); }`,
  );
  ...
  code = expandMacro(
    ...
    "LD16_NNRR",
    ([regl, regh]) =>
      `{ var ldtemp; addTstates(12);` +
      ` ldtemp = readbyte(z80.pc++); z80.pc &= 0xffff;` +
      ` ldtemp |= readbyte(z80.pc++) << 8; z80.pc &= 0xffff;` +
      ` writebyte(ldtemp++, (${regl})); ldtemp &= 0xffff; writebyte(ldtemp, (${regh})); }`,
  );
  ...
  code = expandMacro(
    ...
    ([regl, regh]) =>
      ...
      ` ldtemp = readbyte(z80.pc++); z80.pc &= 0xffff;` +
      ` ldtemp |= readbyte(z80.pc++) << 8; z80.pc &= 0xffff;` +
      ` (${regl}) = readbyte(ldtemp++); ldtemp &= 0xffff; (${regh}) = readbyte(ldtemp); }`,
  );
  ...
  code = expandMacro(
    ...
    ([reg]) =>
      ...
      ` ldtemp = readbyte(z80.pc++); z80.pc &= 0xffff;` +
      ` ldtemp |= readbyte(z80.pc++) << 8; z80.pc &= 0xffff;` +
      ` var regl = readbyte(ldtemp++); ldtemp &= 0xffff; var regh = readbyte(ldtemp);` +
      ` ${reg} = regl | (regh << 8); }`,
  );
  ...
  code = expandMacro(
    ...
    ([regl, regh]) =>
      `{ addTstates(6);` +
      ` (${regl}) = readbyte(z80.sp++); z80.sp &= 0xffff;` +
      ` (${regh}) = readbyte(z80.sp++); z80.sp &= 0xffff; }`,
  );
  ...
  code = expandMacro(
    ...
    ([regl, regh]) =>
      `{ addTstates(6);` +
      ` z80.sp--; z80.sp &= 0xffff; writebyte(z80.sp, (${regh}));` +
      ` z80.sp--; z80.sp &= 0xffff; writebyte(z80.sp, (${regl})); }`,
  );
  ...
  code = expandMacro(
    ...
    () =>
      `{ addTstates(6);` +
      ` var retl = readbyte(z80.sp++); z80.sp &= 0xffff;` +
      ` var reth = readbyte(z80.sp++); z80.sp &= 0xffff;` +
      ` z80.pc = retl | (reth << 8); }`,
  );
  ...
  code = expandMacro(
    ...
    "RL",
    ([v]) =>
      `{ var rltemp = (${v}); (${v}) = (((${v}) & 0x7f) << 1) | (z80.f & FLAG_C);` +
      ` z80.f = (rltemp >> 7) | sz53p_table[(${v})]; }`,
  );
  ...
  code = expandMacro(
    ...
    "RR",
    ([v]) =>
      `{ var rrtemp = (${v}); (${v}) = ((${v}) >> 1) | ((z80.f & 0x01) << 7);` +
      ` z80.f = (rrtemp & FLAG_C) | sz53p_table[(${v})]; }`,
  );
  ...
  code = expandMacro(
    ...
    "SBC16",
    ([v]) =>
      `{ var sub16temp = (z80.l | (z80.h << 8)) - (${v}) - (z80.f & FLAG_C);` +
      ` var lookup = (((z80.l | (z80.h << 8)) & 0x8800) >> 11) | (((${v}) & 0x8800) >> 10) | ((sub16temp & 0x8800) >> 9);` +
      ` z80.h = (sub16temp >> 8) & 0xff; z80.l = sub16temp & 0xff;` +
      ` z80.f = (sub16temp & 0x10000 ? FLAG_C : 0) | FLAG_N | overflow_sub_table[lookup >> 4] |` +
      ` (z80.h & (FLAG_3 | FLAG_5 | FLAG_S)) | halfcarry_sub_table[lookup & 0x07] |` +
      ` ((z80.l | (z80.h << 8)) ? 0 : FLAG_Z); }`,
  );
  ...
  code = expandMacro(
    ...
    ([v]) =>
      ...
      ` z80.a = sbctemp & 0xff;` +
      ` z80.f = (sbctemp & 0x100 ? FLAG_C : 0) | FLAG_N | halfcarry_sub_table[lookup & 0x07] |` +
      ` overflow_sub_table[lookup >> 4] | sz53_table[z80.a]; }`,
  );
  ...
  code = expandMacro(
    ...
    ([v]) =>
      ...
      ` z80.a = subtemp & 0xff;` +
      ` z80.f = (subtemp & 0x100 ? FLAG_C : 0) | FLAG_N | halfcarry_sub_table[lookup & 0x07] |` +
      ` overflow_sub_table[lookup >> 4] | sz53_table[z80.a]; }`,
  );
...
export function generate(datFilePath, register = null) {
  ...
  _run(datFilePath);
  return macroExpand(chunks.join(""), register);
}
